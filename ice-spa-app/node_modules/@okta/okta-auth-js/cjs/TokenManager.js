...
function add(sdk, tokenMgmtRef, storage, key, token) {
  ...
  if (!(0, _util.isObject)(token) || !token.scopes || !token.expiresAt && token.expiresAt !== 0 || !(0, _types.isIDToken)(token) && !(0, _types.isAccessToken)(token)) {
    throw new _AuthSdkError.default('Token must be an Object with scopes, expiresAt, and an idToken or accessToken properties');
  }
...
class TokenManager {
  constructor(sdk, options) {
    ...
    if (options.storage === 'localStorage' && !_browserStorage.default.browserHasLocalStorage()) {
      (0, _util.warn)('This browser doesn\'t support localStorage. Switching to sessionStorage.');
      options.storage = 'sessionStorage';
    }

    if (options.storage === 'sessionStorage' && !_browserStorage.default.browserHasSessionStorage()) {
      (0, _util.warn)('This browser doesn\'t support sessionStorage. Switching to cookie-based storage.');
      ...
      switch (options.storage) {
        ...
        case 'cookie':
          ...
          storageProvider = function (options) {
            var storage = _browserStorage.default.getCookieStorage(options);
            ...
            return {
              ...
              setItem: function (key, value) {
                ...
                Object.keys(existingValues).forEach(k => {
                  _browserStorage.default.storage.delete(key + '_' + k);
                });
              }
            };
          }(sdk.options.cookies);
        ...
        case 'memory':
          storageProvider = _browserStorage.default.getInMemoryStorage();
          break;

        default:
          throw new _AuthSdkError.default('Unrecognized storage option');
      }
    }

    var storageKey = options.storageKey || _constants.TOKEN_STORAGE_NAME;
    var storage = (0, _storageBuilder.default)(storageProvider, storageKey);
